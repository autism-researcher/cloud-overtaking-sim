...
D=agg("out_real/sweep_real.csv","lam")
...
for r in csv.DictReader(open("out_real/asym_real.csv")):
...
for p in ["base","signal","cloud"]:
    ...
    ax.errorbar(x,m,yerr=e,fmt=MRK[p],color=COL[p],capsize=3,lw=2.2,ms=7,
                markeredgecolor="white",markeredgewidth=0.6,elinewidth=1.2,label=LAB[p])
ax.set_xlabel("heavy-direction share of demand [%] (total fixed at 0.16 veh/s)",fontsize=14)
